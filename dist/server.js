...
const glenbikes_typescript_seattle_1 = require("glenbikes-typescript-seattle");
const glenbikes_typescript_test_1 = require("glenbikes-typescript-test");
...
chokidar = require('chokidar'), express = require("express"), fs = require("fs"), license = require("./util/licensehelper"), LocalStorage = require('node-localstorage').LocalStorage, 
//logging = require("./util/logging"),
path = require("path"), Q = require('dynamo-batchwrite-queue'), 
//Q = require('./util/batch-write-queue.js'),
//seattle = require('./opendata/seattle.js'),
soap = require("soap"), strUtils = require('./util/stringutils.js'), uuidv1 = require("uuid/v1");
let seattle = new glenbikes_typescript_seattle_1.SeattleRegion();
...
const MUTEX_TWIT_POST_MAX_HOLD_MS = 100000, MUTEX_TWIT_POST_MAX_RETRIES = 5, MUTEX_TWIT_POST_MAX_WAIT_MS = 300000;
process.setMaxListeners(15);
const MAX_RECORDS_BATCH = 2000, INTER_TWEET_DELAY_MS = process.env.hasOwnProperty("INTER_TWEET_DELAY_MS") &&
...
var log = getLogger(LogType.app), lastdmLog = getLogger(LogType.last_dm), lastmentionLog = getLogger(LogType.last_mention);
...
var botScreenNameRegexp = new RegExp("@" + process.env.TWITTER_HANDLE + "\\b", "i");
...
app.all("/tweet", function (request, response) {
    ...
    getAccountID(T).then((app_id) => {
        try {
            ...
            let tweet_process_promise = processNewTweets(T, docClient, app_id);
...
app.all("/dumpfile", (request, response) => {
...
app.all("/dumptweet", (request, response) => {
    try {
...
app.all("/dumpcitations", (request, response) => {
    try {
        let state;
        let plate;
...
app.all("/errors", (request, response) => {
    try {
        var fileName = `${__dirname}/log/err.log`;
        log.info(`Sending file: ${fileName}.`);
        response.sendFile(fileName);
...
app.all("/processrequests", (request, response) => {
    try {
        var docClient = new AWS.DynamoDB.DocumentClient();
        ...
        GetRequestRecords()
            ...
            if (request_records && request_records.length > 0) {
                request_records.forEach((item) => {
                    let citation_records = [];
                    let tokens = item.license.split(":");
                    let state;
                    let plate;
                    if (tokens.length == 2) {
                        state = tokens[0];
                    ...
                    var request_promise = new Promise((resolve, reject) => {
                        if (state == null || state == "" || plate == null || plate == "") {
                            log.warn(`Not a valid state/plate in this request (${state}/${plate}).`);
                            ...
                            var citation = {
                                ...
                                Citation: glenbikes_typescript_test_1.CitationIds.CitationIDNoPlateFound,
                            ...
                            debugger;
                            batchWriteWithExponentialBackoff(new AWS.DynamoDB.DocumentClient(), tableNames["Citations"], citation_records).then(() => {
                                var params = {
                            ...
                            seattle.GetCitationsByPlate(plate, state).then((citations) => {
                                if (!citations || citations.length == 0) {
                                    ...
                                    var citation = {
                                        id: strUtils._getUUID(),
                                        Citation: glenbikes_typescript_test_1.CitationIds.CitationIDNoCitationsFound,
                                        request_id: item.id,
                                        processing_status: "UNPROCESSED",
                                        license: item.license,
                                        created: now,
                                    ...
                                    citations.forEach(citation => {
                                        ...
                                        // TTL is 10 years from now until the records are PROCESSED
                                        var ttl_expire = new Date(now).setFullYear(new Date(now).getFullYear() + 10);
                                        let citation_record = new interfaces_1.CitationRecord(citation);
...
app.all("/processcitations", (request, response) => {
    try {
        var docClient = new AWS.DynamoDB.DocumentClient();
            ...
            if (citations && citations.length > 0) {
                ...
                Object.keys(citationsByRequest).forEach((request_id) => {
                    var request_promise = new Promise((resolve, reject) => {
                        ...
                        let citation = citationsByRequest[request_id][0];
...
app.all("/processreportitems", (request, response) => {
    ...
    GetReportItemRecords()
        ...
        if (report_items && report_items.length > 0) {
            ...
            Object.keys(reportItemsByRequest).forEach(request_id => {
                reportItemsByRequest[request_id] = reportItemsByRequest[request_id].sort(function (a, b) {
            ...
            Object.keys(reportItemsByRequest).forEach(request_id => {
                var request_promise = new Promise((resolve, reject) => {
                    ...
                    var mutex_client, mutex_broker;
                    Promise.all([new live_mutex_1.LMXBroker().ensure(), new live_mutex_1.LMXClient().connect()]).then(([broker, client]) => {
                        mutex_client = client;
                        mutex_broker = broker;
                        ...
                        SendResponses(mutex_client, T, origTweet, reportItemsByRequest[request_id])
                            .then(() => {
                            log.info(`Finished sending ${reportItemsByRequest[request_id].length} tweets for request ${reportItemsByRequest[request_id][0].request_id}.`);
                            tweet_count =
                            ...
                            batchWriteWithExponentialBackoff(new AWS.DynamoDB.DocumentClient(), tableNames["ReportItems"], report_item_records)
                                .then(() => {
        ...
        Promise.all(request_promises)
            ...
            if (request_promises.length > 0) {
                log.info(`Sent ${tweet_count} tweets for ${reportitem_count} report items.`);
            }
...
function processNewTweets(T, docClient, bot_app_id) {
    ...
    var mentions_promise = new Promise((resolve, reject) => {
        ...
        T.get("search/tweets", {
            ...
            let num_tweets = data.statuses.length;
            let num_request_records = 0;
                ...
                data.statuses.forEach((status) => {
                    ...
                    */
                    ...
                    if (!chomped || botScreenNameRegexp.test(chomped_text)) {
                        ...
                        log.debug("Ignoring reply that didn't actually reference bot: " +
                            status.full_text);
                    }
                    if (request_records.length > 0) {
                        num_request_records += request_records.length;
                        twitter_promises.push(batchWriteWithExponentialBackoff(docClient, tableNames["Request"], request_records));
                    }
                });
            }
            else {
...
function chompTweet(tweet) {
    ...
    if (tweet.display_text_range != null &&
...
function mungeObject(o, propertyOverrides) {
...
function GetQueryCount(license) {
            ...
            T.post("statuses/update", {
                ...
                if (err && response.statusCode != 187) {
                    ...
                    sleep(report_items_clone.length > 0 ? INTER_TWEET_DELAY_MS : 0).then(() => {
                        ...
                        log.debug(`Releasing mutex ${key}...`);
                        if (mutex_client) {
                            mutex_client.release(key, { id: id }).then((v /*LMUnlockSuccessData not exported... but it is*/) => {
                                log.debug(`Released mutex ${key}.`);
...
function getTweetById(T, id) {
    ...
    var promise = new Promise((resolve, reject) => {
        var retTweet;
        T.get(`statuses/show/${id}`, { tweet_mode: "extended" }, (err, tweet, response) => {
            if (err) {
                handleError(err);
                reject(tweet);
            }
        ...
        docClient.query(params, function (err, result) {
            return __awaiter(this, void 0, void 0, function* () {
                if (err) {
                    ...
                    if (result.hasOwnProperty("last_evaluated_key") &&
                        ...
                        var requestIndex = 0;
                        while (citation_records.length < MAX_RECORDS_BATCH &&
                            ...
                            yield docClient.query(params, function (err, result) {
                                if (err) {
                                    handleError(err);
                                }
                                else {
            ...
            return __awaiter(this, void 0, void 0, function* () {
                if (err) {
                    ...
                    if (result.hasOwnProperty("last_evaluated_key") &&
                        ...
                        // until we get MAX_RECORDS_BATCH records.
                        var requestIndex = 0;
                        while (report_item_records.length < MAX_RECORDS_BATCH &&
                            requestIndex < Object.keys(requestIDs).length) {
                            var requestID = requestIDs[Object.keys(requestIDs[requestIndex])];
                            requestIndex++;
